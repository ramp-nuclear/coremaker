...
def make_water(
    *,
    ...
    ρ_w, ρ_w_other = [ρ_w0 * (1.0 - sum(impurities.values())) * frac for frac in (purity, 1.0 - purity)]
    mass_per_molecule = (2 * main_iso.mass + O.mass) / avogadro  # In 1e-24 gr
    mass_per_molecule_other = (2 * other_iso.mass + O.mass) / avogadro
    nd_w, nd_w_other = [ρ / mpm for ρ, mpm in zip((ρ_w, ρ_w_other), (mass_per_molecule, mass_per_molecule_other))]
    wiso = {main_iso: 2 * nd_w, other_iso: 2 * nd_w_other, O: nd_w + nd_w_other}
    aiso = {iso: ρ_w0 * ppm * avogadro / iso.mass for iso, ppm in impurities.items() if ppm > 0.0}
    isos = {iso: wiso.get(iso, 0.0) + aiso.get(iso, 0.0) for iso in unique(chain(wiso, aiso))}
    mix = Mixture(isos, temp, sab_tables=sab_tables)
    return Mixture.expand(mix) if expand_naturals else mix


def make_light_water(
    temp: float,
    impurities: Optional[Dict[Isotope, float]] = None,
    expand_naturals: bool = False,
    sab_tables: SabTables = (Chemical.LightWater, Chemical.HeavyWater),
) -> Mixture:
