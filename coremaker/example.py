...
aluminum_block_size = 8.
aluminum_block_height = 80.
aluminum_block_dimensions = (*([aluminum_block_size] * 2),
                             aluminum_block_height)
# noinspection PyTypeChecker
aluminum_block_node = Node(geometry=Box((0.,) * 3,
                                        dimensions=aluminum_block_dimensions),
                           mixture=aluminium)
...
# noinspection PyTypeChecker
U_block_node = Node(geometry=Box((0.,) * 3,
                                 dimensions=U_block_dimensions),
                    mixture=U_mixture)
U_block_path = aluminum_block_path / PurePath('uranium_block')
# Constructing the tree that represents the fuel rod
fuel_rod_tree = Tree()
fuel_rod_tree.nodes.update({coolant_path: heavy_water_coolant_node,
                            aluminum_block_path: aluminum_block_node,
                            U_block_path: U_block_node})
...
aluminum_grid = CartesianGrid(
    ...
    height=grid_height, mixture=aluminium,
    rod_contents={f'{letter}{index}': deepcopy(fuel_rod_tree)
...
hafnium_block_names = {direction: PurePath(f'{direction}_hafnium_block')
                       for direction in hafnium_transforms.keys()}
# noinspection PyTypeChecker
hafnium_block_nodes = {
    hafnium_block_names[direction]: Node(Box((0.,) * 3, hafnium_block_dimensions, transform=transform), mixture=hafnium)
...
heavy_water_pool_geometry = Box((0.,) * 3, heavy_water_pool_dimensions)
heavy_water_pool_node = Node(geometry=heavy_water_pool_geometry,
                             mixture=heavy_water)
...
hafnium_block_paths = valmap(partial(truediv, heavy_water_pool_name),
                             hafnium_block_names)
core_tree.nodes.update(zip(hafnium_block_paths.values(),
                           hafnium_block_nodes.values()))
# adding the exclusion relations.
core_tree.exclusive = {heavy_water_pool_name: [(aluminum_lattice_path,
