...
matplotlib = pytest.importorskip(
    "matplotlib", reason="Could not import matplotlib. Install this package with the [viz] feature to include it."
)
matplotlib.use("Agg")
...
def site_values(core):
    ...
    return {site: round(1.5 - np.hypot(geoms[site].center_x, geoms[site].center_y) / 60.0, 4) for site in occ}
...
def test_default_width_y_is_none():
    geom = CellGeometry(1.0, 2.0, CellShape.HEXAGON, 5.0)
    assert geom.width_y is None


def test_cell_dimensions_match_lattice(core):
...
def test_site_geometry_returns_cellgeometry(core):
    site = next(iter(core.grid.keys()))
...
def test_all_site_geometries_covers_all_sites(core):
    geoms = all_site_geometries(core)
    all_sites = set(core.grid.sites())
    assert set(geoms.keys()) == all_sites


def test_occupied_sites_subset_of_all(core):
    occ = occupied_sites(core)
    all_sites = set(core.grid.sites())
    assert occ.issubset(all_sites)


def test_site_labels_uses_element_name(core):
...
def test_square_patch_position_and_size(cx, cy, wx, wy):
...
def test_square_patch_default_height_equals_width(cx, cy, wx):
...
def test_hexagon_patch_has_six_vertices():
...
def test_hexagon_patch_centered():
    cx, cy = 3.0, 7.0
...
def test_returns_figure_and_axes(core, site_values):
...
def test_colorbar_present(core, site_values):
...
def test_accepts_existing_axes(core, site_values):
...
def test_labels_hidden_when_disabled(core, site_values):
...
def test_image_regression_for_heatmap(core, site_values, image_regression):
    fig, _ = plot_heatmap(core, site_values, cmap="viridis", title="Regression Test", show_labels=False)
    image_regression.check(_fig_to_png_bytes(fig), diff_threshold=0.1, basename="heatmap_regression")
    plt.close(fig)


def test_legend_contains_all_categories(core, site_categories):
...
def test_image_regression_for_categorical_plot(core, site_categories, image_regression):
    ...
    image_regression.check(_fig_to_png_bytes(fig), diff_threshold=0.1, basename="categorical_regression")
    plt.close(fig)
...
def test_arrows_drawn_for_movements(core):
...
def test_discharge_markers_drawn(core):
...
def test_load_markers_drawn(core):
...
def test_image_regression(core, image_regression):
    plan = TransitionPlan(
    ...
    fig, _ = plot_transition(
        core,
        plan,
        title="Transition Regression",
        load_color_dict={"fresh": "#2ecc71"},
        rod_color_dict={"fuel_rod": "#4a90d9"},
    )
    image_regression.check(_fig_to_png_bytes(fig), diff_threshold=0.1, basename="transition_regression")
    plt.close(fig)


def test_returns_dict_for_occupied_sites(core):
    rotations = site_rotations(core)
    occupied = occupied_sites(core)
    assert set(rotations.keys()) == occupied


def test_values_are_normalised(core):
    rotations = site_rotations(core)
    for angle in rotations.values():
        assert 0 <= angle < 360


def test_unrotated_rods_are_zero(core):
    rotations = site_rotations(core)
    for angle in rotations.values():
        assert angle == pytest.approx(0.0, abs=1e-6)


def test_rotation_map_accepts_existing_axes_to_draw_on(core):
    fig_ext, ax_ext = plt.subplots()
...
def test_arrows_drawn_for_occupied_sites(core):
...
def test_rod_map_without_rotations(core):
...
def test_rod_map_with_rotations(core):
...
def test_empty_plan_has_empty_rotations():
    plan = TransitionPlan()
    assert plan.rotations == {}


def test_plan_with_rotations():
    plan = TransitionPlan(
        movements=[("A1", "B2")],
        rotations={"B2": 90.0},
    )
...
def test_transition_draws_rotation_arrows(core):
...
def test_transition_no_arrows_when_disabled(core):
    from matplotlib.patches import Arc
