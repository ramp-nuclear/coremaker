...
rotations = st.builds(
    Rotation.from_quat, st.tuples(medfloats, medfloats, medfloats, medfloats).filter(lambda x: norm2(x, ord=2) > 1e-3)
)
translations = st.builds(Transform, st.tuples(medfloats, medfloats, medfloats))
transforms = st.builds(Transform.from_rotation, st.tuples(medfloats, medfloats, medfloats), rotations, st.booleans())
...
annuli = st.builds(
    Annulus,
    st.tuples(medfloats, medfloats, medfloats),
    st.tuples(st.shared(posfloats, key="annulus"), st.floats(0, 0.999, exclude_min=True)).map(prod),
    st.shared(posfloats, key="annulus"),
    posfloats,
    st.tuples(medfloats, medfloats, medfloats).filter(lambda x: norm2(x, ord=2) > 1e-3),
)

boxes = st.builds(Box, st.tuples(medfloats, medfloats, medfloats), st.tuples(posfloats, posfloats, posfloats))

balls = st.builds(Ball, st.tuples(medfloats, medfloats, medfloats), medfloats)
hexprisms = st.builds(HexPrism, st.tuples(medfloats, medfloats, medfloats), posfloats, posfloats)

rectangles = st.builds(Rectangle, st.tuples(medfloats, medfloats), st.tuples(posfloats, posfloats))

circles = st.builds(Circle, st.tuples(medfloats, medfloats), medfloats)

rings = st.builds(
    Ring,
    st.tuples(medfloats, medfloats),
    st.tuples(st.shared(posfloats, key="ring"), st.floats(0, 0.999, exclude_min=True)).map(prod),
