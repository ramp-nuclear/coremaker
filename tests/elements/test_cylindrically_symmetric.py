...
def test_radial_split_with_resolution_of_full_annulus_is_split_to_3_in_known_way(inner_radius, radius):
    ...
    assert radial_splitting[0] == inner_radius
    assert np.isclose(
        radial_splitting[1] ** 2 - radial_splitting[0] ** 2, radial_splitting[2] ** 2 - radial_splitting[1] ** 2
    )
    assert radial_splitting[-1] == radius


def test_axial_split_gives_similar_chunks_for_equal_numbers_in_one_example():
    lengths = axial_split(30.0, 30.0)
...
def test_piece_centers_for_one_example_is_as_known():
    lengths = axial_split(30.0, 30.0)
    length = sum(lengths)
    piece_centers = _piece_centers(lengths, (0.0, 0.0, 80.0))
    assert np.isclose(piece_centers[0], [0.0, 0.0, -length / 4]).all()
    assert np.isclose(piece_centers[1], [0.0, 0.0, length / 4]).all()
...
def test_chunked_annulus_tree_for_known_values_gives_known_tree_structure():
    inner_radius, outer_radius, length = 5.0, 10.0, 30.0
    resolution = (outer_radius - inner_radius, length)
    chunked_annulus_tree = ChunkedAnnulusTree(
    ...
    assert all(isinstance(node.geometry, Annulus) for node in chunked_annulus_tree.nodes.values())


def test_chunked_cylinder_tree_for_known_values_gives_known_tree_structure():
    radius, length = 5.0, 30.0
    resolution = (radius, length)
    chunked_cylinder = ChunkedCylinderTree(radius, length, al1050, ROOT_PATH, ZAXIS, resolution)
    ...
    cylinder_nodes = {
        path: node for (path, node) in chunked_cylinder.nodes.items() if isinstance(node.geometry, FiniteCylinder)
    }
    assert len(cylinder_nodes) == 3


def test_appropriate_radial_resolution_splits_annular_ring_according_with_splits_num():
    inner_radius, outer_radius = 5.0, 10.0
...
def test_unequally_chunked_cylinder_tree_for_known_values_gives_known_tree_structure():
    ...
    chunked_cylinder = UnequallyChunkedCylinderTree(
        radius=radius, axis=ZAXIS, mixture=al1050, root_path=ROOT_PATH, length=length, lengths=lengths
    )
    assert isinstance(chunked_cylinder, Tree)
    assert isinstance(chunked_cylinder.outer_geometry, FiniteCylinder)
...
cyls = lengths_list.map(
    lambda x: UnequallyChunkedCylinderTree(
        radius=5,
...
annuli = lengths_list.map(
    lambda x: UnequallyChunkedAnnulusTree(
        outer_radius=5,
        inner_radius=2,
        length=np.sum(x).item(),
        mixture=al1050,
