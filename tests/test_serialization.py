...
geoms = st.one_of(bares, balls, boxgeos, circles, rings, finitecylinders, rectangles,
                  hexprisms, hexagons)
geomseq = st.lists(geoms).map(tuple)
holed = st.builds(ConcreteHoledGeometry, geoms, geomseq, geomseq)
unions = st.builds(ConcreteUnionGeometry, geomseq.filter(lambda x: len(x) > 1),
...
cartlats = st.builds(CartesianLattice, 
                     center=centers,
                     shape=gshape,
                     dimensions=dim2d,
...
hexlats = st.builds(HexagonalLattice,
                    center=centers,
                    shape=gshape,
                    pitch=posfloats,
                    height=st.none() | posfloats,
...
tree4 = ExcludeFrame(frame_dimensions=(4, 4, 4),
...
spacegrids = st.builds(SpacedGrid,
                       center=centers,
...
holes_width = holes.flatmap(lambda x: st.lists(st.tuples(st.integers(min_value=-5, max_value=5), posfloats),
...
genspacegrids = st.builds(GeneralSpacedGrid,
                          center=centers,
                          shape=gshape,
...
tree_aliases = shared_trees.filter(lambda x: x.nodes).flatmap(
        lambda t: st.dictionaries(st.text(ascii_lowercase, min_size=1),
                                  st.tuples(st.just("Explanation"),
