...
example_splitbox_dim = (1.0, 2.0, 3.0)
...
def test_boxtree_has_one_known_component_by_example():
    ...
    assert comp.geometry == Box((0.0, 0.0, 0.0), dim)
...
def test_splitbox_top_has_known_box_geometry_by_example(_example_splitbox):
    ...
    assert roots[0][1].geometry == Box((0.0, 0.0, 0.0), example_splitbox_dim)
...
def _example_framebox() -> FrameBox:
    ...
    return FrameBox(
        frame_dimensions=example_splitbox_dim,
...
def test_framebox_pieces_have_correct_mixtures_by_example(_example_framebox):
    frames = filter(lambda x: "FramePiece" in str(x[0]), _example_framebox.named_components())
    pictures = filter(lambda x: "MooFrame/MooPicture/" in str(x[0]), _example_framebox.named_components())
...
def test_framebox_subdicts_all_start_at_root(_example_framebox):
    ...
    for d in (_example_framebox.inclusive, _example_framebox.exclusive, _example_framebox.external_exclusive):
...
def test_framebox_picture_has_known_box_geometry_by_example(_example_framebox, _example_excludeframe):
    for tree in (_example_excludeframe, _example_framebox):
        picture = tree[PurePath("MooFrame") / "MooPicture"]
        assert picture.geometry == Box((0.0, 0.0, 0.0), example_inner_dim)


def test_excludebox_pieces_have_correct_mixtures_by_example(_example_excludeframe):
    frames = filter(lambda x: "MooFrame" == str(x[0]), _example_excludeframe.named_components())
    pictures = filter(lambda x: "MooFrame/MooPicture/" in str(x[0]), _example_excludeframe.named_components())
    assert all(comp.mixture == steel_304L for _, comp in frames)
    assert all(comp.mixture == al6061 for _, comp in pictures)
...
def test_singular_construction_has_correct_suffixes_by_example():
    ...
    tree = singular_root_construction(
        tuple(zip(3 * [factory], transforms, suffixes)), root_path=PurePath("root"), relationship=ChildType.inclusive
    )
...
def test_exclude_to_frame_makes_an_equivalent_frame_by_example():
    ...
    trans_framebox = excludeframe_to_framebox(
        exclude, picture_name=PurePath("MooPicture"), frame_resolution=outer_resolution
    )
