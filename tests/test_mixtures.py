...
temperatures = st.floats(min_value=0.0, max_value=400.0)
densities = st.floats(min_value=1e-6, max_value=2.0)
...
@given(isodicts, keys)
def test_mixture_closeness_with_small_density_change(isodict: dict[ZAID, float], key: ZAID):
...
zaids = st.tuples(
    st.integers(min_value=0), st.integers(min_value=0, max_value=998), st.integers(min_value=0, max_value=9)
).map(lambda x: ZAID(x[0], x[1], x[2]))
number_densities = st.floats(min_value=1e-10, max_value=1.0, allow_nan=False, allow_infinity=False)
...
def test_mixture_inequality_when_temperature_is_different(isotopes: Dict[ZAID, float], temps: Tuple[float, float]):
    t1, t2 = temps
...
@given(isotope_dicts, st.floats(allow_nan=False), chemical_shuffled_list, chemical_shuffled_list)
...
shared_dicts = st.shared(st.dictionaries(keys=zaids, values=number_densities, min_size=1), key="dictmoo")
perm_dicts = shared_dicts.flatmap(lambda x: st.permutations(list(x.items()))).map(lambda x: dict(x))
...
def test_room_condition_helium_gas_has_low_but_existing_density_He4():
    helium = {He4: 1}
    gas = ideal_gas_mixture(helium, 1.0, 20.0)
...
recursive_tuple = st.recursive(
    st.dictionaries(element_names, nums, min_size=1).map(lambda x: tuple(x.items())), extend_tuple, max_leaves=20
)
...
def _recursive_tuple_to_dictionary(x):
    ...
    return {Isotope.from_name(key): value for key, value in Counter(re.findall("[A-Z][a-z]?", flatten_tup(x))).items()}
...
def test_chemical_parsing_mixtures_succeeds_and_adds_up(forms, isod):
    d = forms | isod
    mix = Mixture.by_chemical_formula(d, 20.0)
